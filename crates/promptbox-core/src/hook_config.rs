...
pub(crate) fn has_stale_promptbox_hook(
    ...
    match root {
        Value::String(value) => {
            command_matches_promptbox(value, provider) && !command_matches_expected(value, expected_command)
        }
        Value::Array(items) => items
            .iter()
            .any(|value| has_stale_promptbox_hook(value, expected_command, provider)),
...
pub(crate) fn remove_promptbox_hooks(root: &mut Value, provider: &str) {
    match root {
        Value::Array(items) => {
            ...
            items.retain(|value| !is_promptbox_command_hook(value, provider) && !empty_hook_entry(value));
        }
        Value::Object(object) => {
